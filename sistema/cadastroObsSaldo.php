...
if ($acesso) {
    ...
    # Nome do Modelo
    $objeto->set_nome('Observação Geral do Saldo');

    # select do edita
    $objeto->set_selectEdita('SELECT obsSaldo
                                FROM tbcontrato
    ...
    $tipo = $modalidade->getTipo($dados["idModalidade"]);
    if ($tipo == "Despesa") {
        $objeto->set_linkListar('cadastroPagamento.php');
        $objeto->set_voltarForm('cadastroPagamento.php');
    } else {
        $objeto->set_linkListar('cadastroReceita.php');
        $objeto->set_voltarForm('cadastroReceita.php');
    }
